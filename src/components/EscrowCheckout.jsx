...
export default function EscrowCheckout({ cartItems = [], onCancel, onConfirmPayment, onNavigate }) {
  ...
  // --- REPLACED HANDLEPAYSTACKPAYMENT FUNCTION ---
  const handlePaystackPayment = () => {
    ...
    // Function to actually open the Paystack popup modal
    const launchPaystack = () => {
      try {
        ...
        alert("Could not open Paystack checkout. Please check your browser ad-blocker or switch to Bank Transfer.");
      }
    };

    // If Paystack is already loaded globally, launch it immediately
    if (window.PaystackPop) {
      launchPaystack();
    } else {
      // Otherwise, load the script dynamically on demand
      ...
      script.onload = () => {
        if (window.PaystackPop) {
          ...
          setLoading(false);
          alert("Paystack SDK loaded but popup object is unavailable.");
        }
      };
      script.onerror = () => {
        setLoading(false);
        alert("Failed to load Paystack SDK. Please check your network connection or use Direct Bank Transfer.");
      };
    ...
    try {
      if (db) {
        try {
          ...
          console.warn("Firestore sync skipped or restricted:", firestoreErr);
        }
      }
